 ...
 *  - 此外每次push完成都要去设置max，设置max的规则是去比较当前的max和当前数组的长度，取出最大值，因此这样取出来的就是出现的最长的字符串的长度
 *
 * 时间复杂度为O(n^2)，因为`indexOf`本质上也是遍历，它的时间复杂度也是O(n)
 * 空间复杂度就是维护的数组和max变量，max是常量，数组是O(n)
 */
 ...
 * 空间复杂度就是维护的数组和max变量，max是常量，数组是O(n)
 */
const _lengthOfLongestSubstring = function (s) {
  // let index = 0
  let max = 0
 ...
 * map结构为`{'a':0,'b':1,'c':2,'a':3,c:'4'}`
 * 所以利用`i`做为起始下标的标记，`j`作为截止下标的标记
 * 当`map`的`get`方法匹配到当前`map`结构中已经存在了当前值，就把`i`设置为当前值的下一项
 * 其实就是利用map的get方法是O(1)的时间复杂度
 * 空间复杂度就是维护的数组和max变量，max是常量，数组是O(n)
 */
const __lengthOfLongestSubstring = function (s) {
  const map = new Map()
  let max = 0
  for (let i = 0, j = 0; j < s.length; j++) {
    if (map.has(s[j])) {
